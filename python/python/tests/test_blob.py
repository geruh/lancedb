...
def testBlobColumnPathsIncludeListChildren():
    schema = pa.schema(
        [
    ...
    assert blob_column_paths(schema) == [
        "info.blob",
        "images.image",
...
def testBlobColumnPathsIncludeMetadataMarkedFields():
    schema = pa.schema(
        [
            pa.field(
    ...
    assert blob_column_paths(schema) == ["legacy", "packed"]
...
def testFetchBlobsRejectsLegacyV1Column():
    ...
    table = db.create_table("legacy_v1", schema=schema)
