...
def test_with_row_id_false_keeps_blob_projection_clean(tmp_db):
    table = tmp_db.create_table("test_with_row_id_false_blob", _blob_query_data())

    hits = table.search().select(["blob"]).limit(1).with_row_id(False).to_arrow()

    assert "_rowid" not in hits.column_names
    assert "_lance_row_id" not in hits.schema.field("blob").type.names
