...
class AsyncQueryBase(object):
    ...
    def with_row_id(self, with_row_id: bool = True) -> Self:
        """
        Include the _rowid column in the results.

        Passing ``False`` is not the same as saying nothing. A blob projection
        adds a row id on its own when the caller has no opinion, and an explicit
